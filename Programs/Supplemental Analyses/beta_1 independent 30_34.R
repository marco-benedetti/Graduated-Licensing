...
saveRDS(main_MCMC,"R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_beta_1_independent_30_34.rds")
# IF YOU WANT TO JUST READ IN THE EXISTING OUTPUT:
#main_MCMC <- readRDS("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_beta_1_independent_30_34.rds")
...
plot((police_data$officers_per*1000),point_est,pch=18,ylim=c(0.8,1.0),xlab="Number of Full Time Police Officers per 1000 16-17 Uear-old Licensed Drivers",ylab="DID Estimate")
for(i in 1:length(counties)){
...
plot((police_data$budget/100000),point_est,pch=18,ylim=c(0.8,1.0),xlab="Total Police Budget (100k)",ylab="DID Estimate")
for(i in 1:length(counties)){
...
plot((police_data$budget_per),point_est,pch=18,ylim=c(0.8,1.0),xlab="Total Police Budget per 16-17 Uear-old Licensed Drivers",ylab="DID Estimate")
for(i in 1:length(counties)){
...
boxplot(point_est~UIC$UIC3,ylab = "DID Point Estimate",xlab="UIC",ylim=c(0.8,0.95))
