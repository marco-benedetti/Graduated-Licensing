...
# READ IN DATA 
basedata_trt <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\OhioInj_1617.csv")
basedata_ctrl <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\OhioInj_1824.csv")
...
plot(analysis_data$year[which((analysis_data$county == counties[1]) & (analysis_data$AGEGRP=="16-17"))],
     ...
     type="l",ylim=c(0,60),xlab="Year",ylab="Crash Rate per 1,000 Licensed Drivers",col=2)
lines(analysis_data$year[which((analysis_data$county == counties[1]) & (analysis_data$AGEGRP=="20-24"))],
...
legend("topright",lty=c(1,1),col=c(2,4),legend=c("16-17","18-24"),cex=1.5)
...
saveRDS(main_MCMC,"R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_injury.rds")

# IF YOU WANT TO JUST READ IN THE EXISTING OUTPUT:
# main_MCMC <- readRDS("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_injury.rds")
