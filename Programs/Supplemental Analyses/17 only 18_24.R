...
# READ IN DATA 
basedata_trt <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\OhioCrashes_17.csv")
...
# Convert variables 
LD_data$LD_17 <- as.numeric(LD_data$LD_17)
...
LD_data_keep <- LD_data[which(LD_data$year %in% seq(2003,2017)),c(1:6,19:21)]
...
trt_data_step3$AGEGRP = rep("17",nrow(trt_data_step3))
trt_data_step3$trt = rep(1,nrow(trt_data_step3))
trt_data_step3$period <- as.numeric(trt_data_step3$year > 2007)
trt_data_step3$injury_rate <- trt_data_step3$COUNT/trt_data_step3$LD_17*1000
...
# Create exposures
analysis_data_step1$treatment_time <- as.numeric(analysis_data_step1$AGEGRP=="17")*2008
...
nimble_data$offset <- ifelse(analysis_data$AGEGRP=="18-24",analysis_data$LD_18_24,analysis_data$LD_17)
...
# CRASH RATES #
# TREATED GROUP #
analysis_data_17 <- analysis_data[which(analysis_data$AGEGRP=="17"),]
overall_trt <- sum(analysis_data_17$COUNT)/sum(analysis_data_17$LD_17)*1000
pre_trt <- sum(analysis_data_17$COUNT[which(analysis_data_17$year < 2008)])/sum(analysis_data_17$LD_17[which(analysis_data_17$year < 2008)])*1000
post_trt <- sum(analysis_data_17$COUNT[which(analysis_data_17$year >= 2008)])/sum(analysis_data_17$LD_17[which(analysis_data_17$year >= 2008)])*1000

diff_trt <- sum(analysis_data_17$COUNT[which(analysis_data_17$year < 2008)])/sum(analysis_data_17$LD_17[which(analysis_data_17$year < 2008)])*1000-
sum(analysis_data_17$COUNT[which(analysis_data_17$year >= 2008)])/sum(analysis_data_17$LD_17[which(analysis_data_17$year >= 2008)])*1000
...
saveRDS(main_MCMC ,"R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_17_only.rds")

# IF YOU WANT TO JUST READ IN THE EXISTING OUTPUT:
# in_MCMC <- readRDS("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_17_only.rds")
