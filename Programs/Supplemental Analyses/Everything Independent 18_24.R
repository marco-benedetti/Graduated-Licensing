...
# READ IN DATA 
basedata_trt <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\OhioCrashes_1617.csv")
basedata_ctrl <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\OhioCrashes_1824.csv")

LD_data <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\Num_LD_Main_File.csv")

pop_00_09 <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\age_grp_ohio_county_pop_00_09.csv")
pop_10_18 <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\age_grp_ohio_county_pop_10_18.csv")

poverty_2008 <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\est08all.csv")
UIC <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\Ohio_UIC.csv")

county_map <- st_read("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\shapefile\\cb_2018_us_county_500k.shp")
ohio_map <- county_map[which(county_map$STATEFP=="39"),]
rm(county_map)

police_data <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Data\\police_data.csv")
...
############################
# TOY FREQUENTIST ANALYSIS #
############################

point_est_F <- rep(0,length(counties))
se_F <- rep(0,length(counties))

nimble_data$logoffset <- log(nimble_data$offset)
nimble_data$county <- analysis_data$county
for(i in 1:length(counties)){
	tmp <- data.frame(nimble_data)[which(nimble_data$county == counties[i]),]
	model <- glm(y ~ trt + law + I04 + I05 + I06 + I07 + I08 + I09 + 
			 I10 + I11 + I12 + I13 + I14 + I15 + I16 + I17 + offset(logoffset),
			 family=poisson(link="log"),data = tmp)
	point_est_F[i] <- summary(model)$coefficients[3,1]
	se_F[i] <- summary(model)$coefficients[3,2]
	
}

plot(exp(point_est_F),ylim=c(0.6,1.4),pch=18,ylab="DID Estimate",xlab = "Arbitraty County Order")

for(i in 1:length(counties)){
	segments(i,exp(point_est_F[i]+se_F[i]),i,exp(point_est_F[i]-se_F[i]))
	segments(i-0.2,exp(point_est_F[i]-se_F[i]),i+0.2,exp(point_est_F[i]-se_F[i]))
	segments(i-0.2,exp(point_est_F[i]+se_F[i]),i+0.2,exp(point_est_F[i]+se_F[i]))
}
