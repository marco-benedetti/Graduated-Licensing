...
# READ IN DATA 
basedata_trt <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\OhioCrashes_16.csv")
basedata_ctrl <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\OhioCrashes_1824.csv")

LD_data <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\Num_LD_Main_File.csv")

pop_00_09 <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\age_grp_ohio_county_pop_00_09.csv")
pop_10_18 <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\age_grp_ohio_county_pop_10_18.csv")

poverty_2008 <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\est08all.csv")
UIC <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\Ohio_UIC.csv")

county_map <- st_read("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\shapefile\\cb_2018_us_county_500k.shp")
ohio_map <- county_map[which(county_map$STATEFP=="39"),]
rm(county_map)

police_data <- read.csv("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Data\\police_data.csv")
...
# Convert variables 
LD_data$LD_16 <- as.numeric(LD_data$LD_16)
...
LD_data_keep <- LD_data[which(LD_data$year %in% seq(2003,2017)),c(1:5,19:21)]
...
trt_data_step3$AGEGRP = rep("16",nrow(trt_data_step3))
trt_data_step3$trt = rep(1,nrow(trt_data_step3))
trt_data_step3$period <- as.numeric(trt_data_step3$year > 2007)
trt_data_step3$injury_rate <- trt_data_step3$COUNT/trt_data_step3$LD_16*1000
...
# Create exposures
analysis_data_step1$treatment_time <- as.numeric(analysis_data_step1$AGEGRP=="16")*2008
...
nimble_data$offset <- ifelse(analysis_data$AGEGRP=="18-24",analysis_data$LD_18_24,analysis_data$LD_16)
...
# CRASH RATES #
# TREATED GROUP #
analysis_data_16 <- analysis_data[which(analysis_data$AGEGRP=="16"),]
overall_trt <- sum(analysis_data_16$COUNT)/sum(analysis_data_16$LD_16)*1000
pre_trt <- sum(analysis_data_16$COUNT[which(analysis_data_16$year < 2008)])/sum(analysis_data_16$LD_16[which(analysis_data_16$year < 2008)])*1000
post_trt <- sum(analysis_data_16$COUNT[which(analysis_data_16$year >= 2008)])/sum(analysis_data_16$LD_16[which(analysis_data_16$year >= 2008)])*1000

diff_trt <- sum(analysis_data_16$COUNT[which(analysis_data_16$year < 2008)])/sum(analysis_data_16$LD_16[which(analysis_data_16$year < 2008)])*1000-
sum(analysis_data_16$COUNT[which(analysis_data_16$year >= 2008)])/sum(analysis_data_16$LD_16[which(analysis_data_16$year >= 2008)])*1000
...
saveRDS(main_MCMC ,"R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Code\\Final Programs\\Output\\Output_16_only.rds")

# IF YOU WANT TO JUST READ IN THE EXISTING OUTPUT:
#main_MCMC <- readRDS("R:\\RESZhu\\Research_Marco\\Policy Effect Heterogeneity\\Send for Code Check\\Output\\Output_16_only.rds")
