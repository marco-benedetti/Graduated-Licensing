...
# Subset to 16-17 and 30-34
pop_data_step2 <- pop_data_step1[which(pop_data_step1$AGEGRP %in% c(4,7)),]
...
ctrl_data_step3 <- merge(ctrl_data_step2,pop_data[which(pop_data$age==7),],by = c("county","year"))
...
lines(analysis_data$year[which((analysis_data$county == counties[1]) & (analysis_data$AGEGRP=="30-34"))],
     analysis_data$injury_rate[which((analysis_data$county == counties[1]) & (analysis_data$AGEGRP=="30-34"))],col=4)
...
pre_ctrl <- sum(analysis_data_30_34$COUNT[which(analysis_data_30_34$year < 2008)])/sum(analysis_data_30_34$TOT_POP[which(analysis_data_30_34$year < 2008)])*1000
post_ctrl <- sum(analysis_data_30_34$COUNT[which(analysis_data_30_34$year >= 2008)])/sum(analysis_data_30_34$TOT_POP[which(analysis_data_30_34$year >= 2008)])*1000

diff_ctrl <- sum(analysis_data_30_34$COUNT[which(analysis_data_30_34$year < 2008)])/sum(analysis_data_30_34$TOT_POP[which(analysis_data_30_34$year < 2008)])*1000-
sum(analysis_data_30_34$COUNT[which(analysis_data_30_34$year >= 2008)])/sum(analysis_data_30_34$TOT_POP[which(analysis_data_30_34$year >= 2008)])*1000
...
saveRDS(main_MCMC ,"C:\\Users\\bened\\Documents\\GDL Paper\\Code\\Upload to Github\\Output\\Output_pop_offset_30_34.rds")
